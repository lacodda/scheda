...
> **Status: `v0.3.2` — the notepad, and a place to live in a long note.** Markup drawn over the source, embedded pictures, a reading mode; an outline, folded sections, and the edits a list expects. Plus the installer, file associations, tabs in the window's own title bar, find and replace, recent files and themes. The file tree and `[[links]]` come next; see the [roadmap](#roadmap).
...
| 0.4 – 0.7 | The vault root rule, links and backlinks, search and index, sessions and a command palette. |
